...
server.onRequest = function (req, res) {
  // Plugin interception: allow plugins to handle the request before normal flow
  for (const plugin of this.plugins) {
    if (typeof plugin.processRequest === 'function') {
      try {
        const handled = plugin.processRequest(req, res);
        if (handled) return;
      } catch (e) {
        util.log('Error in plugin.processRequest:', e);
      }
    }
  }
